...
$$
f(x,y)=x^{2}+xy^{2}+y^{3}
$$$$\frac{\partial f}{\partial x} = (x^{2}+y^{2}x+y^{3})_{x}^{'} = 2x+y^{2}$$$$\frac{\partial f}{\partial y} = (x^{2}+xy^{2}+y^{3})_{y}^{'} = 0+2xy + 3y^{2}$$

## 9.2
$$ z = x^4 + 3x^2 y^5 $$$$\frac{dz}{dx} = (x^4 + 3x^2 y^5)_x = 4x^3 + 6x y^5$$
...
## 9.3. Теорема (необходимые условия дифф. фyн.)
Пусть функцию $z=f(x,y)$ дифф. в точке $М_о(х_о,у_о)$. Тогда она непрерывна в этой точке и имеет в ней частиные производные по обеим независимым переменным 
Примем 
$$f_x'(x_o, y_o) = A $$
$$fy'(xo, yo) = B $$
$z = f(x,y)$ - диф. в г. $А_о$ => $∆Z(Mo) = ADX + BAY +α1∆X +α2∆y$
$$
$$
...
## 9.6$$ \text {Теорема 1 о производной сложной функции}$$

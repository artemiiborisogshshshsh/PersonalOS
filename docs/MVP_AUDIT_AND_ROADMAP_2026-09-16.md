...
### 2026-09-23 — общий invite-only вход, локальная вертикаль

- Добавлен запускаемый `scripts/invited_beta_bot.py`: операторские invite/revoke,
  private sender validation, отдельные state и Google token paths, повторное
  использование изолированного pilot runtime. Старый entrypoint сохранён.
- Доступ и операция защищены одной per-user блокировкой; revoke завершается после
  in-flight операции. Поколение приглашения сбрасывает старые подтверждения.
  Durable per-user и общий polling cursor (включая отклонённые updates)
  обеспечивают at-most-once dispatch; после crash нужна новая
  команда preview, восстановление записей идёт через существующие journals.
- Synthetic-проверки покрывают двух пользователей, onboarding → preview → apply,
  revoke, чужие кнопки, restart, частичную запись и restore, ручные изменения,
  устаревшие исходные данные и отказы провайдеров. Live-проверка остаётся оператору.
- Ограничение вертикали: последовательный polling; отдельный token file не
  доказывает различие Google-аккаунтов (это проверяет оператор при provisioning).
- **Пункт 2 открыт**: следующим этапом реализовать версионированный diff-preview
  опубликованного плана поверх существующих reconciliation/projection journals,
  повторную проверку владения и Calendar перед apply, явное разрешение удаления
  и конфликтов, восстановление частичного изменения. Пока изменения опубликованного
  плана безопасно блокируются. Пункты 3–5 не расширяли текущую реализацию.

- Целевая локальная проверка общего входа и пилотных регрессий: **92 passed**
  (11 новых synthetic-тестов общего входа + 81 существующая проверка); это не
  полный suite. Отдельный regression исключает replay отклонённого `/start`
  после restart и последующего приглашения.


### 2026-09-24 — подтверждаемое обновление публикации, локальная вертикаль

- Общий invite-only вход включает `PublishedPlanUpdates`: preview добавлений и
  переносов пар/подготовок, версия плана и явное подтверждение, повторное чтение
  источника и Calendar перед записью. Изолированный pilot остаётся фиксированным.
- Два необязательных поля `DraftOperation` хранят опубликованный снимок и pending
  update в существующем store. Старые snapshots читаются; без baseline обновления
  блокируются, автоматической миграции нет. Backup/restore сохраняет новые поля.
- Запись использует прежние projection journals и verified writers. ETag +
  условный PATCH защищают от ручной правки между чтением и обновлением; strict
  insert conflict больше не превращается в безусловный update. Неоднозначный UID
  останавливает запись. Журнал предшествует каждой мутации.
- После crash точное уже записанное состояние принимается без дубля. Если
  попытка вставки была начата, а удалённого события нет, продолжение блокируется:
  невозможно доказать, что пользователь не удалил успешно созданное событие.
- По orchestrate один независимый reviewer проверил только эту вертикаль;
  подтверждённые дефекты strict insert/restart/отсутствующего ETag исправлены.
- Целевая локальная проверка: **171 passed**, включая реальный `main` с synthetic
  провайдерами, обновления, регрессии пилота, backup/restore и projection recovery.
  Это не полный suite и не live-проверка Google/Telegram/TPU.
- **Осталось по пунктам 1–2:** изоляция зависших запросов пользователей,
  подтверждаемые удаления с доказуемым основанием, явное разрешение конфликтов,
  безопасная работа с уже наступившими событиями и сменой недели. Следующий этап —
  пользовательский разбор конфликта/исчезновения перед отдельным разрешением
  удаления; не расширять работу до задач/tutoring/новых интеграций.


### 2026-09-24 — явный разбор исчезновения и подтверждаемое удаление

- Общий бот получил `/review_missing`: конкретные пары/подготовки, выбор оставить
  сейчас или подтвердить удаление перечисленных. Обычный preview ничего не
  удаляет; исчезновение не принимается за автоматическое разрешение.
- Цели ограничены опубликованными занятиями с текущим доказательством из
  проверенного источника (UID отсутствует или статус явно cancelled) и их
  подготовками. Устаревшее локальное CANCELLED само по себе недостаточно.
- Подтверждение связано с версией и исходными данными, истекает через 10 минут.
  Перед удалением проверяется полный baseline, UID, Google ID, владение, ETag;
  DELETE использует If-Match. Ручные изменения и неоднозначности блокируют запись.
- Продолжение после crash использует существующий pending journal и новый
  `/review_missing`. Уже удалённые события, включая tombstone с тем же ID,
  принимаются без повторного DELETE; другой ID блокирует продолжение. Backup и
  restore покрыты. Возвращение пары после удаления создаёт только новый preview.
- Один независимый reviewer по orchestrate проверил эту вертикаль; замечание
  о tombstone закрыто проверкой точного ID и регрессионным тестом.
- Целевая локальная выборка: **193 passed** (команда в runbook), включая настоящий
  entrypoint с synthetic-провайдерами, удаление/восстановление и прежние регрессии.
  Это не полный suite, реальные провайдеры не запускались.
- Остались: явное разрешение ручных конфликтов, другие основания удаления,
  уже наступившие события/смена недели, изоляция зависших запросов пользователей.
  Следующий законченный этап — принятие ручного переноса/удаления как решения
  пользователя с безопасным пересчётом остального плана.

### 2026-09-25 — принятие ручных переносов и удалений Calendar

- Общий invite-only бот получил `/review_calendar`: показывает конкретное
  расхождение и последствия, затем принимает решение только в локальном state.
  Остальной план рассчитывается через отдельный `/weekly_preview` и записывается
  только после второго подтверждения. Изолированный pilot не изменён.
- Принятое время пары или подготовки сохраняется. Удалённые вручную события не
  восстанавливаются; новые подготовки к удалённой паре не создаются. Уже
  опубликованные связанные подготовки сохраняются и резервируют свои интервалы.
- Решения находятся в `published_plan.manual_resolutions` существующего store;
  backup/restore и pending projection journals продолжают использовать прежний
  формат. Проверяются источник, весь baseline, владение, Google ID и ETag.
- Поддерживаются временные переносы и доказуемые удаления. Изменения других
  полей, неоднозначности, переносы в прошлое/за горизонт/с пересечениями и
  возвращение вручную удалённого события требуют оператора. Это безопасная
  вертикаль, а не универсальное разрешение всех конфликтов Calendar.
- Реальные Telegram, TPU и Google не запускались. Live-drill и команды описаны
  в `CLOSED_BETA_RUNBOOK.md`; synthetic-проверки не заменяют проверку провайдеров.
- Итоговая целевая выборка: **224 passed in 32.50s**, не полный suite.
  Независимое ревью выявило два дефекта (удалённая подготовка при исчезновении
  пары и версия tombstone); оба исправлены и закрыты повторной проверкой.
- Следующий самостоятельный этап — изоляция зависших запросов разных
  пользователей. Уже наступившие события, смена недели и другие основания
  удаления остаются открыты; задачи/tutoring и новые интеграции не включены.
